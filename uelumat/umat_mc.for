...
!-----------------------------------------------------------------------
      ...
      do i=1,ntens
         write(200,201)(ddsdde(i,j),j=1,ntens)
      end do
c     
c     recover stress and strain from statev array
c
        do k1=1,ntens
          stress(k1)=statev(k1+4)
          stran(k1) =statev(k1+ntens+4)
        end do
c     
        do k1=1,ntens
           do k2=1,ntens
...
c
        do k1=1,ntens
          statev(k1+4)=stress(k1)
          statev(k1+ntens+4)=stran(k1) 
        end do
201   format(f12.2,2x,f12.2,2x,f12.2,2x,f12.2,2x)
!-----------------------------------------------------------------------
      end subroutine UMAT
!-----------------------------------------------------------------------
      pure subroutine MohrCoulombStressReturn(Sigma,nsigma,PlasPar,
